...
public class RestAccessBean implements Serializable {
	...
	public RestAccessBean() throws ServletException, IOException,
			NamingException {
		Context ctx = new InitialContext();
		ds = (DataSource) ctx.lookup(GlobalService.JNDI_DB_NAME);
	}

	public Collection<RestBean> getCompany() throws SQLException {
		String queryPageString = "SELECT id, name  FROM  eBookCompany ";
		Collection<RestBean> coll = new ArrayList<RestBean>();
	...
	public String getSelectTag() throws SQLException {
		String ans = "";
		Collection<RestBean> cb = getCompany();
		ans += "<SELECT name='" + getTagName() + "'>";
		for (RestBean bean : cb) {
			int id = bean.getId();
